...
const picker=document.getElementById('pick-audio');if(picker)picker.addEventListener('click',async()=>{const response=await fetch('/picker/audio',{method:'POST'});if(!response.ok)return;const data=await response.json();if(data.cancelled)return;document.getElementById('selection-id').value=data.selection_id;document.getElementById('selected-name').textContent=data.filename;document.getElementById('import-panel').hidden=false;});
