...
const picker=document.getElementById('pick-audio');if(picker)picker.addEventListener('click',async()=>{const dialog=document.getElementById('import-dialog');if(!dialog)return;dialog.showModal();const response=await fetch('/picker/audio',{method:'POST'});if(!response.ok)return;const data=await response.json();if(data.cancelled){dialog.close();return;}document.getElementById('selection-id').value=data.selection_id;document.getElementById('selected-name').textContent=data.filename;});
/* 导入弹窗：类型切换控制 RJ 号字段显隐（仅 RJ 作品有 RJ 号） */
const kindSelect=document.getElementById('kind');if(kindSelect){const rjField=document.querySelector('[data-rj-field]');const syncKind=()=>{if(rjField)rjField.style.display=kindSelect.value==='rj_work'?'':'none';};kindSelect.addEventListener('change',syncKind);syncKind();}
