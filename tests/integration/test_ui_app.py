...
def test_fixed_token_persists(tmp_path):
    app = create_app(UiDependencies(
        settings=UiSettingsStore(tmp_path / "ui.json"),
        picker=FakeFilePicker(),
        profiles=ModelProfileStore(tmp_path / "profiles.json"),
        worker=FakeWorkerAdapter([]),
        startup_token="my-fixed-secret",
        is_fixed_token=True,
        open_browser=False,
        allowed_hosts={"testserver"},
    ))
    client = TestClient(app)
    # First login with fixed token succeeds
    resp1 = client.get("/?token=my-fixed-secret", follow_redirects=False)
    assert resp1.status_code == 303
    # Clear cookies on client to simulate fresh browser session
    client.cookies.clear()
    # Subsequent login with the same fixed token still succeeds
    resp2 = client.get("/?token=my-fixed-secret", follow_redirects=False)
    assert resp2.status_code == 303


def test_no_auth_mode_allows_access_without_token(tmp_path):
    app = create_app(UiDependencies(
        settings=UiSettingsStore(tmp_path / "ui.json"),
        picker=FakeFilePicker(),
        profiles=ModelProfileStore(tmp_path / "profiles.json"),
    ...
    client = TestClient(app)
    # Accessing root without token automatically redirects to set session cookie
    resp = client.get("/", follow_redirects=False)
    assert resp.status_code == 303
    assert "subforge_session" in resp.cookies
    # Following redirect allows viewing the page
    resp_page = client.get("/")
    assert resp_page.status_code == 200
    # Direct access to subpage without prior session cookie succeeds and sets cookie
    fresh_client = TestClient(app)
    resp_sub = fresh_client.get("/about")
    assert resp_sub.status_code == 200
    assert "subforge_session" in resp_sub.cookies
...
def test_index_renders_work_card_grid_like_asmr_one(tmp_path):
    ...
    assert '/library/rescan' in html
    css = (REPO_ROOT / "subforge" / "ui" / "static" / "app.css").read_text(encoding="utf-8")
...
def test_track_row_click_opens_player_and_menu_buttons_do_not_navigate(tmp_path):
    ...
    script = (REPO_ROOT / "subforge" / "ui" / "static" / "app.js").read_text(encoding="utf-8")
...
def test_video_info_endpoint(tmp_path, monkeypatch):
    """/api/video-info 端点：提取视频元数据用于导入自动补全。"""
    library = tmp_path / "Library"
    client, headers = _authenticated_client(tmp_path, library=library)

    # 缺少 url
    assert client.get("/api/video-info", headers=headers).status_code == 400

    # Mock _fetch_video_info
    import subforge.ui.app as app_mod
    monkeypatch.setattr(app_mod, "_fetch_video_info", lambda url, proxy="": {
        "ok": True,
        "title": "测试视频",
        "author": "测试UP主",
    ...
    resp = client.get("/api/video-info?url=https://www.bilibili.com/video/BV1xx411c7mD", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["ok"] is True
    assert data["title"] == "测试视频"
    assert data["author"] == "测试UP主"

    # 别名 /api/url-info
    resp_alias = client.get("/api/url-info?url=https://www.bilibili.com/video/BV1xx411c7mD", headers=headers)
    assert resp_alias.status_code == 200
    assert resp_alias.json()["title"] == "测试视频"


def test_upload_image_endpoint(tmp_path):
    """/api/upload-image 接收浏览器直接上传图片并返回 selection_id。"""
    library = tmp_path / "Library"
    client, headers = _authenticated_client(tmp_path, library=library)

    # 缺少文件内容
    resp_empty = client.post("/api/upload-image", headers=headers, content=b"")
    assert resp_empty.status_code == 400

    # 上传正常图片（直接二进制流）
    fake_img = b"\xff\xd8\xff\xe0fakejpegcontent"
    h = dict(headers)
    h["Content-Type"] = "image/jpeg"
    h["X-Filename"] = "my_cover.jpg"
    resp = client.post("/api/upload-image", headers=h, content=fake_img)
    assert resp.status_code == 200
    data = resp.json()
    assert "selection_id" in data
    assert data["filename"] == "my_cover.jpg"
    assert data["preview_url"].startswith("/api/selections/")
...
def test_import_url_without_author_falls_back_gracefully(tmp_path, monkeypatch):
    """当未指定 author 且 creator_ids 为空时，导入应自动兜底作者，避免抛出 author is required 异常。"""
    library = tmp_path / "Library"
    fake_audio = tmp_path / "downloaded.m4a"
    fake_audio.write_bytes(b"\x00" * 2048)
    ...
    from subforge.ui.app import _download_and_import
    monkeypatch.setattr("subprocess.Popen", _fake_popen)
    monkeypatch.setattr("shutil.which", lambda name: "yt-dlp" if name == "yt-dlp" else None)
    ...
    store = LibraryStore.initialize(library)
    # author 为 None，不应抛出 ValueError("author is required for stream archive")
    result = _download_and_import(
        store, "https://www.youtube.com/watch?v=test",
        kind=ItemKind.STREAM_ARCHIVE, rj_code=None, title=None, author=None,
    )
    store.close()
    assert result.created is True
    item = LibraryStore.open(library).get_item(result.item_id)
    assert item.author is not None and len(item.author) > 0
...
def test_segment_toolbar_time_chip_and_bilingual_mode_sync_assets():
    ...
    js = (REPO_ROOT / "subforge" / "ui" / "static" / "player.js").read_text(encoding="utf-8")
...
def test_subtitle_edit_dialog_has_reprocess_entry_and_larger_layout():
    ...
    js = (REPO_ROOT / "subforge" / "ui" / "static" / "player.js").read_text(encoding="utf-8")
    html = (REPO_ROOT / "subforge" / "ui" / "templates" / "player.html").read_text(encoding="utf-8")
    css = (REPO_ROOT / "subforge" / "ui" / "static" / "app.css").read_text(encoding="utf-8")
...
def test_is_bilibili_url():
    from subforge.ui.app import _is_bilibili_url
    assert _is_bilibili_url("https://www.bilibili.com/video/BV1xx411c7mD") is True
    assert _is_bilibili_url("https://b23.tv/abcd123") is True
    assert _is_bilibili_url("https://www.youtube.com/watch?v=12345") is False


def test_fetch_bilibili_video_info_handles_lowercase_bv_and_protocol_relative_pic(monkeypatch):
    import subforge.ui.app as app_mod
    ...
    class FakeClient:
        ...
        def get(self, url, **kwargs):
            assert "BV1xx411c7mD" in url  # Verified lowercase bv got capitalized to BV
            return FakeResponse({
                "code": 0,
                "data": {
                    "title": "B站视频",
    ...
    info = app_mod._fetch_video_info("https://www.bilibili.com/video/bv1xx411c7mD")
    assert info["ok"] is True
    assert info["title"] == "B站视频"
    assert info["author"] == "UP主"
    assert info["cover_url"] == "https://i0.hdslb.com/bfs/archive/pic.jpg"
