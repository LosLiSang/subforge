...
def test_global_player_repairs_missing_item_id_when_same_track_reactivated():
    js = (REPO_ROOT / "subforge" / "ui" / "static" / "global-player.js").read_text(encoding="utf-8")
...
def test_global_player_has_persistent_300_percent_volume_control():
    js = (REPO_ROOT / "subforge" / "ui" / "static" / "global-player.js").read_text(encoding="utf-8")
...
def test_shell_exposes_shared_player_spacing_and_accent_presets(tmp_path):
    ...
    css = (REPO_ROOT / "subforge" / "ui" / "static" / "theme-overrides.css").read_text(encoding="utf-8")
    js = (REPO_ROOT / "subforge" / "ui" / "static" / "app.js").read_text(encoding="utf-8")
...
def test_player_bar_supports_seamless_spa_navigation(tmp_path):
    """底部播放栏支持 HTMX SPA 无刷新跳转，避免全页刷新导致播放器销毁与卡顿。"""
    client, track_id = _player_client(tmp_path)
    shell = client.get("/", headers={"sec-fetch-dest": "document"}).text
    assert 'id="player-bar"' in shell
    assert 'hx-boost="true"' in shell
    assert 'hx-target="#main-content"' in shell

    js = (REPO_ROOT / "subforge" / "ui" / "static" / "global-player.js").read_text(encoding="utf-8")
    assert "window.htmx.ajax" in js or "htmx.ajax" in js
    assert "player-bar-cover" in js
    assert "player-bar-title" in js
    assert "openPlayPage" in js or "navigateTo" in js
    assert "htmx.process(bar)" in js

    player_js = (REPO_ROOT / "subforge" / "ui" / "static" / "player.js").read_text(encoding="utf-8")
    assert "htmx.ajax" in player_js

    app_js = (REPO_ROOT / "subforge" / "ui" / "static" / "app.js").read_text(encoding="utf-8")
    assert "track-row[data-track-player]" in app_js
    assert "htmx.ajax" in app_js
